      ...
      <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 space-y-6 sm:space-y-8 max-w-5xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-neutral-200 shadow-sm p-6 sm:p-8">
              <h3 className="text-lg font-bold text-[#0A1B39] mb-6">Pricing Information</h3>
              <div className="grid grid-cols-2 gap-6">
                <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-100">
                  <p className="text-xs text-[#676E85] mb-1 font-medium">Current Price</p>
                  <p className="text-xl sm:text-2xl font-bold text-[#0A1B39]">₦{(course.price / 100).toLocaleString()}</p>
                </div>
                <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-100">
                  <p className="text-xs text-[#676E85] mb-1 font-medium">Original Price</p>
                  <p className="text-xl sm:text-2xl font-bold text-[#98A2B3] line-through">₦{course.originalPrice ? (course.originalPrice / 100).toLocaleString() : "N/A"}</p>
                </div>
                <div className="col-span-2 p-4 rounded-xl bg-[#17A546]/5 border border-[#17A546]/20">
                  <p className="text-xs text-[#17A546] mb-1 font-semibold">Total Revenue Generated</p>
                  <p className="text-xl sm:text-2xl font-bold text-[#17A546]">₦0</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border border-neutral-200 shadow-sm overflow-hidden">
              ...
              <div className="p-4">
                <div className="p-4 rounded-xl bg-neutral-50 border border-neutral-200 flex items-start gap-3 mb-4">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-[#0A1B39] truncate">
                      {course.pdfPath ? "course-material.pdf" : "No PDF uploaded"}
                    </p>
                    <p className="text-xs text-[#676E85] mt-0.5">PDF Document</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <Button 
                    variant="outline" 
                    className="w-full border-neutral-200 text-[#0A1B39] font-semibold h-9 text-xs rounded-md shadow-sm"
                    onClick={() => setIsPreviewOpen(true)}
                    disabled={!course.pdfPath}
                  >
                    <Eye className="h-3.5 w-3.5 mr-2" />
                    Preview
                  </Button>
                  <Button variant="outline" className="w-full border-neutral-200 text-[#0A1B39] font-semibold h-9 text-xs rounded-md shadow-sm" disabled>
