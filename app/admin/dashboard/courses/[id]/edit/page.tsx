...
const EditCoursePage = ({ params }: { params: Promise<{ id: string }> }) => {
  ...
  useEffect(() => {
    if (course) {
      ...
      setCourseData({
        ...
        standardPrice: course.standardPrice ? (course.standardPrice / 100).toString() : "",
        premiumPrice: course.premiumPrice ? (course.premiumPrice / 100).toString() : "",
  ...
  const handleSave = async (status: "active" | "draft") => {
    ...
    try {
      ...
      // Basic validation for pricing (convert to kobo)
      ...
      // Payload for updateCourseRequest
      const payload: any = {
              ...
              <div className="md:col-span-2 space-y-6">
                <div className="bg-white p-6 sm:p-8 rounded-xl border border-neutral-200 shadow-sm space-y-6">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-bold text-[#0A1B39] mb-1">Basic Tier Price (₦)</label>
                      <p className="text-[11px] text-[#676E85] mb-2">Reading notes & PDF access (Required)</p>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-sm font-bold text-[#0A1B39] mb-1">Standard Tier Price (₦)</label>
                      <p className="text-[11px] text-[#676E85] mb-2">Notes + Topic Videos (Optional)</p>
                      <input
                        type="number"
                        placeholder="e.g. 8000 (leave empty if disabled)"
                        className="w-full bg-neutral-50/50 border border-neutral-200 rounded-lg px-4 py-3 outline-none focus:border-[#17A546]/40 focus:ring-2 focus:ring-[#17A546]/20 transition-all text-[#0A1B39] font-medium"
                        value={courseData.standardPrice}
                        onChange={(e) => setCourseData({ ...courseData, standardPrice: e.target.value })}
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#0A1B39] mb-1">Premium Tier Price (₦)</label>
                      <p className="text-[11px] text-[#676E85] mb-2">All Access & Priority Support (Optional)</p>
                      <input
                        type="number"
                        placeholder="e.g. 12000 (leave empty if disabled)"
                        className="w-full bg-neutral-50/50 border border-neutral-200 rounded-lg px-4 py-3 outline-none focus:border-[#17A546]/40 focus:ring-2 focus:ring-[#17A546]/20 transition-all text-[#0A1B39] font-medium"
                        value={courseData.premiumPrice}
                        onChange={(e) => setCourseData({ ...courseData, premiumPrice: e.target.value })}
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-bold text-[#0A1B39] mb-1">Original Price Anchor (₦)</label>
                      <p className="text-[11px] text-[#676E85] mb-2">Reference slash-through price (Optional)</p>
                      <input
                        type="number"
