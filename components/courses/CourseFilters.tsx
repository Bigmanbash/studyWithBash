...
export function CourseFilters({ activeFilter, onFilterChange }: CourseFiltersProps) {
  return (
    <div className="flex items-center gap-1.5 overflow-x-auto pb-2 sm:pb-0 scrollbar-hide bg-(--card) p-1.5 rounded-2xl shadow-[0_2px_10px_-4px_rgba(0,0,0,0.05)] border border-border/80 w-full sm:w-auto">
      {levels.map((level) => (
        <button
          key={level.id}
          ...
          className={`px-5 py-2.5 rounded-xl text-sm font-semibold whitespace-nowrap transition-all duration-300 relative ${
            activeFilter === level.id
              ...
              : "text-(--muted) hover:bg-[rgba(255,255,255,0.08)] hover:text-(--heading)"
          }`}
        >
          {activeFilter === level.id && (
            <div className="absolute inset-0 bg-linear-to-r from-[#17A546] to-[#14933E] rounded-xl -z-10" />
          )}
